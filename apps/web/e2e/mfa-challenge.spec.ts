...
test.describe("MFA Challenge Flow", () => {
  test.skip("shows MFA challenge after login when user has MFA enabled", async ({
  ...
  test("MFA challenge form validates code length", async ({
    authenticatedPage: page,
  }) => {
  ...
  test("dashboard loads after successful MFA verification", async ({
    authenticatedPage: page,
  }) => {
    // This test requires:
    // 1. A test user with MFA enabled
