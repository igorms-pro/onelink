  ...
  const enroll = useCallback(async () => {
    await submit(async () => {
      ...
      if (result.error) {
        ...
        if (is422) {
          ...
          if (
            ...
            // No factor data in response - try to clean up any unverified factors we can find
      ...
      if (!factor.id) {
        ...
        toast.error("Failed to get factor ID. Please try again.");
        throw new Error("Factor has no ID");
      }

      console.log("[MFA] Factor ID set in state:", factor.id);
  ...
  const verifyEnrollment = useCallback(
    async (code: string) => {
      ...
      await submit(async () => {
        ...
        if (!factorId) {
          // State was lost - can't recover without localStorage
        ...
        console.log("[MFA] Verify response received:", {
          ...
          errorMessage:
            verifyResult.error instanceof Error
              ? verifyResult.error.message
              : String(verifyResult.error),
          errorType: verifyResult.error?.constructor?.name,
        });

        if (verifyResult.error) {
          console.error("[MFA] ❌ Verification failed:", verifyResult.error);

          // Check if error is because factor doesn't exist (user deleted it via SQL)
          const errorMsg =
            verifyResult.error instanceof Error
              ? verifyResult.error.message
              : String(verifyResult.error);
          const isFactorNotFound =
        ...
        console.log("[MFA] Clearing enrollment state...");
        setEnrollingFactor(null);
        setQrCodeData("");
        setState("active");
        console.log("[MFA] Reloading factors...");
        await loadFactors();
